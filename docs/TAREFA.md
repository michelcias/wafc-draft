...
| E1.10 terminologia | não aberta; **pode abrir já**, prioridade baixa | catálogo da §3 |
...
| E1.10 | trocar o termo **"sieve"** nos `derivations/` (D33), com a mesma disciplina de E1.4c: **só o vocabulário**, nenhum enunciado, constante, hipótese ou número muda. Ocorrências hoje: `02` (10), `03` (1), `04` (7), `05` (13), `06` (3), `07` (1), mais uma em `macros.tex` (comentário). A substituição é "espaço de aproximação" nos arquivos em português e "approximation space" onde o termo aparece em inglês, com a forma longa "expansão em base de dimensão crescente" onde ela ler melhor; onde o termo for essencial para quem vem da literatura teórica, pode ficar **uma vez**, entre parênteses. Recompilar os `.pdf` tocados. **Não tocar no manuscrito**: `ms_1.tex` e `supp_1.tex` entram pela rodada de `k = 2` (pergunta 15 do `ESTADO.md`) | D33 | `derivations/02-aproximacao-besov.tex`, `derivations/03-desenho-produtos.tex`, `derivations/04-oraculo.tex`, `derivations/05-taxas.tex`, `derivations/06-selecao-limiar.tex`, `derivations/07-rota-intervalo.tex`, `derivations/macros.tex` (só o comentário), os `.pdf` correspondentes, `docs/handoff-E1.10.md` |
...
**Nada em curso. Abertas no catálogo: E2.4b e E1.10**, que não compartilham
arquivo (`wafc/` e `derivations/`). **E2.5 (go/no-go) só deve abrir depois de E2.4b**, por
