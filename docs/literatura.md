...
| **Xue & Yang (2006)**, *Statistica Sinica* 16(4) 1423–1446, "Additive coefficient modeling via polynomial spline" | o additive coefficient model; splines; **taxa ótima univariada sob `α_ls ∈ C^{p+1}[0,1]`** (Teorema 1), com centralização `E{α_ls(X_s)} = 0` à la Stone e recentralização empírica na eq. (3.4); BIC para seleção | a hipótese `C^{p+1}` é **a classe em que splines são ótimos** e exclui o alvo do WAFC (Besov com `π < 2`), de modo que a afirmação teórica do artigo não é "taxa melhor na classe deles". Mas teorema que não cobre um caso não diz que o método vá mal nele, nos dois sentidos: o WAFC **pode ganhar no suave**, porque escolhe `J` e `λ` por validação cruzada e zera os níveis finos desnecessários, enquanto o spline de nós equiespaçados gasta graus de liberdade uniformemente. É medição de E2.4 e E4, não conversa; sem wavelets; sem LASSO | `verificado` (`Xue-Yang-2006`; sem DOI: a SS não registrou 2006 no Crossref; conferido no PDF da editora) |
