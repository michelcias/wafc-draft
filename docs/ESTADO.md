...
| D31 | 09-20 | **Em avaliação numérica repetida, a base é fixada e avaliada por tabela**, não pelo algoritmo de Daubechies-Lagarias a cada ajuste: construir a tabela uma vez com `WaveBased::wtable()` para o par `(family, filter.size)` e passá-la em `wavelet.table` de `wafc_design()`, em vez de deixar a regra `use.table = "auto"` decidir réplica a réplica. Vale para o piloto (E2.4), o compêndio (E4) e a aplicação (E6). **Exceção:** conferência que mede precisão fina (as de `derivations/check/`, que leem decaimento até `1e-11`) usa avaliação exata ou tabela com `prec.wavelet` alto, porque o erro de interpolação medido em E2.1 é de `3.1e-06` e engoliria o que se quer medir | pedido do autor; além do custo, é reprodutibilidade: com `auto` (que dispara em `n q ≥ 2000 L`) parte das réplicas usaria tabela e parte avaliação exata, e a diferença entre as duas entraria nos resultados como ruído sistemático |
