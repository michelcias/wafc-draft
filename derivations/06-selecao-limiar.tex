...
relação com $S$: sob a Hipótese de Besov de E1.3 o oráculo do sieve é
genericamente denso (E1.6, ``onde este resultado entra''), e na prática o
LASSO deixa coeficientes não nulos em blocos nulos --- a conferência de E2.2
...
\begin{itemize}[nosep, leftmargin=*]
  ...
  \item \textbf{O alvo limiarizado é $\PiJ\gcomp{\cv}{\md}$, não
    $\gcomp{\cv}{\md}$.} A estatística vive na resolução $J_{n}$, e o viés do
    sieve entra em $D_{n}$ pela terceira parcela de \eqref{eq:Dn}. Nas
...
\begin{itemize}[nosep, leftmargin=*]
  ...
  \item \textbf{Parte III, a janela e o acerto.} $\sepmin = 1$ nos dois
    ...
    do Corolário~\ref{cor:limiar}(ii) contra (iii): o viés do sieve encolhe
    todas as normas na mesma direção e não cria falso positivo.
