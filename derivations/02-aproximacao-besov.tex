...
para toda wavelet (Meyer, 1992, cap.~III, \S 11; Daubechies, 1992, \S 9.3
\textsc{[verificar]}). É a base que o \texttt{wbasis()} devolve com
...
\S 2) e de Härdle et al.\ (1998, cap.~9 \textsc{[verificar]}). A
...
$1$-periódica está em $\Besov{s}{\pi}{r}$ localmente (Meyer, 1992, cap.~VI;
Cohen, 2003, cap.~3 \textsc{[verificar]}). A implicação de que a prova
...
(Triebel, 1983; Cohen, 2003, cap.~3 \textsc{[verificar]}) e usando a
...
\begin{itemize}[nosep]
  ...
  \item Com \texttt{boundary = "interval"} (Cohen, Daubechies e Vial, 1993),
    ...
    2003, cap.~3 \textsc{[verificar]}); o Lema~\ref{lem:L2} vale então com
...
Referências fora de \texttt{docs/referencias-verificadas.bib} estão marcadas
\textsc{[verificar]} e vão ao handoff.
...
\begin{itemize}[nosep]
  ...
  \item Daubechies, I. (1992). \emph{Ten Lectures on Wavelets}. SIAM.
    \textsc{[verificar]}
  ...
  \item Härdle, W., Kerkyacharian, G., Picard, D. e Tsybakov, A. (1998).
    ...
    Notes in Statistics 129. Springer. (\texttt{hardle1998wavelets} no WALL;
    não está no \texttt{.bib} verificado) \textsc{[verificar]}
  \item Meyer, Y. (1992). \emph{Wavelets and Operators}. Cambridge Studies in
    Advanced Mathematics 37. Cambridge University Press. \textsc{[verificar]}
  ...
  \item Triebel, H. (1983). \emph{Theory of Function Spaces}. Birkhäuser.
    \textsc{[verificar]}
\end{itemize}
