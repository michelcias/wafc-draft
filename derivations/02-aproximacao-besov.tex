% E1.3 -- lema de aproximação em Besov no espaço gerado por wavelets
% periodizadas.
...
\title{E1.3 e E1.3b. Aproximação em Besov no espaço gerado por wavelets
...
efetiva $\seff = s - (1/\pi - 1/2)_{+}$ apareça (é ela que separa a aproximação
...
\paragraph{Espaço de aproximação (\emph{sieve}) e projeção.}
$\WJ = \operatorname{span}\{\wav{\lev}{\tsl} : 0 \le \lev < J\}$, de dimensão $\NJ = 2^{J} - 1$, é o complemento ortogonal
...
\]
a sua aproximação em $\WJ$, com $\coefs{\cv}{\md}{\lev}{\tsl} =
...
\begin{corollary}[viés de aproximação]\label{cor:vies}
...
\begin{itemize}[nosep]
  ...
  \item O reescalonamento para $[\varepsilon, 1-\varepsilon]$ do
    ...
    E1.5 usa é $\inf$ sobre $\WJ$ da norma $L_{2}(P_{U})$, e não
...
\begin{enumerate}[label=\textup{(\roman*)}, nosep]
  ...
  \item e o viés de aproximação obedece à cota do Corolário~\ref{cor:vies} com
...
\begin{itemize}[nosep]
  \item A aproximação \emph{não linear}. Para $\pi < 2$ a melhor aproximação
    com $M$ termos escolhidos livremente atinge $M^{-s}$, enquanto a
    aproximação linear atinge só $\NJ^{-\seff}$; é essa diferença que o LASSO
    explora e que o corolário de compressibilidade (E1.6, weak-$\ell_{\tau}$ com
    $\tau = (s + 1/2)^{-1}$ no caso crítico) formaliza. O lema aqui é só o
    viés do espaço de aproximação cheio.
...
\begin{itemize}[nosep]
  ...
  \item bumps (Donoho e Johnstone), com $\seff = 3/2$ pelas quinas
    ...
    pré-assintótico, e é também a situação em que o LASSO ganha da
    aproximação linear. Uma quina periódica isolada, $|\sin\pi(u - 0.4)|$, dá quedas que oscilam
