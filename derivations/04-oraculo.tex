...
% especificado dentro do espaço de aproximação (o viés de E1.3 entra no lado
% direito), e a constante de desenho vem de E1.4 na forma de autovalor cheio,
% o que dispensa a condição de cone. Arquitetura de passos transposta do WALL teórico
...
$f$ pela sua aproximação $f_{J}$ em $\WJ$ (Corolário~\ref{cor:vies-E13} abaixo
...
\begin{enumerate}[label=(\alph*), leftmargin=*, nosep]
  ...
  \item o modelo é \textbf{mal especificado dentro do espaço de
    aproximação}: $f \notin \operatorname{span}$ das colunas de $\bZ$, e o
    resíduo determinístico $\bb = f - f_{J}$ entra como um segundo termo de
    ruído, tratado por Young em vez de por dualidade.
...
para $\lev < J$, de modo que $f_{J} = \bZ\betastar$ é a aproximação em $\WJ$.
...
Note que $s_{0}$ é a esparsidade \emph{do oráculo do espaço de aproximação},
não uma hipótese sobre o modelo: sob a Hipótese de Besov de E1.3 os
$\coefs{\cv}{\md}{\lev}{\tsl}$ decaem mas não se anulam, e em geral $s_{0} = d$. É por isso que a taxa lenta
...
\begin{corollary}[viés de aproximação; provado em E1.3]\label{cor:vies-E13}
...
\begin{itemize}[nosep, leftmargin=*]
  \item \textbf{A norma populacional.} Tudo aqui é em $\normn{\cdot}$, a norma
    ...
    uniforme sobre a classe de funções do espaço de aproximação e fica em E1.6.
