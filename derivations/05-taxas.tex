...
oráculo do sieve é genericamente denso}: os $\coefs{\cv}{\md}{\lev}{\tsl}$
decaem mas não se anulam, e $s_{0} = |\Szero| = d$. Lido no oráculo com
$s_{0} = d$, o Teorema~1(ii) de E1.5 dá a taxa do sieve de (b); a única
...
aproximação não linear $n^{-2s/(2s+1)}$ onde o sieve linear só alcança
...
\]
o oráculo do sieve $\betastar = (\bc,\thetastar)$ com
...
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*]
  \item Sob as Hipóteses de base e de Besov de E1.3, com
    ...
    automático quando $\pi \ge 2$ ---, o oráculo do sieve
...
\begin{theorem}[taxa de predição do sieve]\label{thm:taxa}
...
\begin{enumerate}[label=\textup{(\roman*)}, leftmargin=*]
  ...
  \item \textup{(a resolução)} Tome $J_{n} = \lceil c\log_{2}n\rceil$ com $c$
    ...
    $2^{-2J_{n}\seff} = O\bigl((J_{n}/n)^{(2-\tau)/2}\bigr)$ (o viés do sieve
    não domina); e a condição empírica de E1.4. Fixado um tal $c$,
    \[
...
$\seff = s - (1/\pi - 1/2)_{+}$ é a regularidade que o \emph{sieve linear}
enxerga (E1.3, Lema~2); $s$ é a que a \emph{aproximação não linear} enxerga.
O Teorema~\ref{thm:taxa} é a taxa do sieve; o Corolário~\ref{cor:compressivel}
é a do LASSO, que escolhe os termos. Para $\pi \ge 2$ as duas coincidem, e é
...
Lido no oráculo, o Teorema~1(ii) de E1.5 tem $s_{0} = d$ e entrega a taxa do
sieve do Teorema~\ref{thm:taxa}; lido num comparador esparso, pelo
Lema~\ref{lem:comparador}, entrega a taxa comprimível. É a mesma desigualdade
...
\begin{itemize}[nosep, leftmargin=*]
  ...
  \item \textbf{Novo:} a Proposição~\ref{prop:lenta}(ii), que mostra que para
    ...
    $(J_{n}/n)^{2s/(2s+1)}$ do sieve \emph{sem nenhuma condição de desenho}.
...
\begin{itemize}[nosep, leftmargin=*]
  ...
  \item \textbf{A norma populacional.} Tudo continua em $\normn{\cdot}$; a
    ...
    sobre a classe do sieve e não é feita nem aqui nem em E1.5. Para as
    componentes, o Corolário~\ref{cor:componentes-taxa} \emph{é} em
...
Hipótese de Besov em todo nível, de modo que o viés do sieve é exato. O que
saiu em 2026-09-19 está registrado em \texttt{docs/handoff-E1.6.md}; os
